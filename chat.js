...
function select_receiver(connection, chatID, senderID) {
    var receiver = "";
    return new Promise(function (resolve, reject) {
        connection.query("SELECT `userIIID` AS receiver from  `tb_chats` WHERE `userIID` = " + senderID + " and `chatID` = " + chatID, function (err, result, fields) {
            if (err) return reject(err);
            else if (result.length == 0) {
                connection.query("SELECT `userIID` AS receiver from `tb_chats` WHERE `userIIID` = " + senderID + " and `chatID` = " + chatID, function (err, res, fields) {
                    if (err) return reject(err);
                    resolve(res)
                });
            }
...
/*
var receiver;
select_receiver(connection, 6, 9).then(function(result){
